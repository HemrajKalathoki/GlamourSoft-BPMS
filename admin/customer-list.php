...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .bpms-table tbody tr:nth-child(8)  { animation-delay: .24s; }
        @keyframes rowIn {
        ...
        .phone-cell i { color: var(--primary); font-size: .85rem; }
        ...
        .email-link i { color: var(--primary); font-size: .8rem; }
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box">
        ...
        <p>You're about to permanently delete <span class="modal-name" id="modalCustomerName">this customer</span>. This action cannot be undone.</p>
        <div class="modal-actions">
            <button class="modal-cancel" onclick="closeModal()">Cancel</button>
            <a href="#" class="modal-confirm" id="confirmDeleteBtn">Yes, Remove</a>
        </div>
    </div>
</div>
...
<div class="dashboard-wrapper" id="dashboard-wrapper">
    <div class="page-content">
        ...
        <div class="page-header">
            <div class="page-header-left">
                ...
                <div class="page-header-text">
                    <h1>Customer List</h1>
                    <p>Manage all registered customers and assign beauty services.</p>
                </div>
            </div>
        </div>

        <?php
        /* Fetch all customers — zero logic change */
        $ret        = mysqli_query($con, "SELECT * FROM tbluser ORDER BY ID DESC");
        $totalCust  = mysqli_num_rows($ret);

        /* Count those who registered today */
        $todayRet   = mysqli_query($con, "SELECT COUNT(*) as cnt FROM tbluser WHERE DATE(RegDate) = CURDATE()");
        $todayRow   = mysqli_fetch_array($todayRet);
        ...
        <div class="stat-strip">
            <div class="stat-chip">
                <div class="stat-chip-icon pink"><i class="bi bi-people-fill"></i></div>
                <div>
                    <div class="stat-chip-val"><?php echo $totalCust; ?></div>
                    <div class="stat-chip-lbl">Total Customers</div>
                </div>
            </div>
            <div class="stat-chip">
                <div class="stat-chip-icon green"><i class="bi bi-person-check-fill"></i></div>
                <div>
                    <div class="stat-chip-val"><?php echo $todayCount; ?></div>
                    <div class="stat-chip-lbl">Joined Today</div>
                </div>
            </div>
            <div class="stat-chip">
                <div class="stat-chip-icon blue"><i class="bi bi-scissors"></i></div>
                <div>
                    <div class="stat-chip-val">
                        <?php
                        $svcRet = mysqli_query($con, "SELECT COUNT(DISTINCT UserID) as cnt FROM tblbook");
                        $svcRow = mysqli_fetch_array($svcRet);
                        echo $svcRow['cnt'];
                        ?>
                    </div>
                    <div class="stat-chip-lbl">With Appointments</div>
                </div>
            </div>
        </div>
        ...
        <div class="toolbar">
            ...
            <div class="result-count" id="resultCount">
                Showing <strong><?php echo $totalCust; ?></strong>
                customer<?php echo $totalCust == 1 ? '' : 's'; ?>
            </div>
        </div>
        ...
        <div class="bpms-card">
            <div class="bpms-card-header">
                ...
                <h5>All Customers</h5>
                <span class="card-badge"><?php echo $totalCust; ?> records</span>
            </div>

            <div style="overflow-x:auto;">
                <table class="bpms-table" id="customerTable">
                    <thead>
                        <tr>
                    ...
                    ?>
                        <tr class="customer-row"
                            ...
                            <td>
                                <div class="name-cell">
                                    ...
                                    <div>
                                        ...
                                        <div class="name-sub">Customer</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="actions">
                                    <a href="add-customer-services.php?addid=<?php echo $row['ID']; ?>"
                                       class="btn-assign">
                                    ...
                                    <button class="btn-delete"
                                        onclick="confirmDelete(
                                            'customer-list.php?delid=<?php echo $row['ID']; ?>',
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="empty-state">
                                    ...
                                    <p>No customers have registered yet.<br>They'll appear here once they sign up.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
...
searchInput.addEventListener('input', function () {
    ...
    resultCount.innerHTML =
        'Showing <strong>' + visible + '</strong> customer' + (visible === 1 ? '' : 's');
});

/* ── Custom delete modal ───────────────────────────────── */
function confirmDelete(url, name) {
    document.getElementById('confirmDeleteBtn').href = url;
    document.getElementById('modalCustomerName').textContent = name || 'this customer';
